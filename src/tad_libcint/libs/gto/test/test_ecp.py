...
mol = gto.M(
    ...
    basis={
        "Na": "lanl2dz",
        "H": [
            [0, [1.21, 1.0], [0.521, 1.0]],
            [1, [3.12, 1.0], [0.512, 1.0]],
            [2, [2.54, 1.0], [0.554, 1.0]],
            [3, [0.98, 1.0], [0.598, 1.0]],
                ...
                for lmb in range(lmax + 1):
                    if (i + j + k + lmb) % 2 == 0:
                        ...
                        for n, (i1, j1, k1) in enumerate(loop_cart(lmb)):
                            tmp += omega_nuc[lmb][n] * int_unit_xyz(
                                i + i1, j + j1, k + k1
                            )
...
def so_by_shell(mol, shls, ecpatm_id, ecpbas):
    ...
    for ic in range(nci):
        for jc in range(ncj):
            ...
            s = lib.PauliMatrices * 0.5j
            gspinor[ic, jc] = numpy.einsum(
                "sxy,spq,xpi,yqj->ij", s, g1[ic, jc], ui.conj(), uj
            )
...
def sph_in(l, xs):
    ...
    return numpy.asarray(
        [scipy.special.spherical_in(numpy.arange(l + 1), x) for x in xs]
    ).T
...
def angular_moment_matrix(l):
    ...
    for mi in range(-l, l + 1):
        mj = mi + 1
        if mj <= l:
            lx[l + mi, l + mj] = 0.5 * ((l + mj) * (l - mj + 1)) ** 0.5
            ly[l + mi, l + mj] = 0.5j * ((l + mj) * (l - mj + 1)) ** 0.5
...
def ang_nuc_part(l, rij):
    ...
    if l == 0:
        ...
        fc2s(
            omega.ctypes.data_as(ctypes.c_void_p),
            ctypes.c_int(1),
            omega_xyz.ctypes.data_as(ctypes.c_void_p),
            ctypes.c_int(l),
        )
        return omega
...
def c2s_bra(l, gcart):
    if l == 0:
        ...
        fc2s = libecp.CINTc2s_ket_sph
        fc2s(
            gsph.ctypes.data_as(ctypes.c_void_p),
            ctypes.c_int(m),
...
class KnownValues(unittest.TestCase):
    ...
    def test_type2(self):
        ...
        def gen_type2(shls):
            ...
            libecp.ECPtype2_sph(
                mat1.ctypes.data_as(ctypes.c_void_p),
                (ctypes.c_int * 2)(*shls),
                mol._ecpbas.ctypes.data_as(ctypes.c_void_p),
...
"""
        ...
        def gen_so(shls):
            ...
            mat2 = 0.5 * gto.ecp.so_by_shell(mol, shls)
