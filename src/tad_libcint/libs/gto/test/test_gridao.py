...
for i in range(natm):
    ...
    atm[i, PTR_COORD] = off
    env[off + 0] = 0.2 * (i + 1)
    env[off + 1] = 0.3 + (i + 1) * 0.5
    env[off + 2] = 0.1 - (i + 1) * 0.5
    off += 3
off0 = off
# basis with kappa > 0
nh = 0
...
class KnownValues(unittest.TestCase):
    ...
    def test_ipsp_spinor(self):
        ...
        rs = (rs - 0.5) ** 2 * 30
        ao1 = eval_gto(
            mol1, "GTOval_ip_spinor", rs, comp=3, shls_slice=(0, mol1.nbas // 2)
        )
        ao2 = eval_gto(
            mol1,
            "GTOval_ipsp_spinor",
            rs,
            comp=3,
            shls_slice=(mol1.nbas // 2, mol1.nbas),
        )
        self.assertAlmostEqual(abs(ao1 - ao2 * 1j).sum(), 0, 9)
...
H  0.  0.  1.3""",
    ...
    def test_int1e_grids_ip(self):
        ...
        ref = df.incore.aux_e2(mol1, fmol, intor="int3c2e_ip1_cart").transpose(
            0, 3, 1, 2
        )
